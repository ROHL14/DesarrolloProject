...
$("document").ready(function () {
  // Cliquea un objeto para check in
  $("table#items-check tbody tr").click(function () {
    var t = $(this);
    var id = $(this).data("id");

    if (actpage == "in") var text = "check-in";
    else var text = "check-out";
    ...
    if (
      ...
      var cont = $(
        '<tr style="display:none" data-id="y"><td></td><td></td><td></td><td><form method="post" data-saveid="' +
          id +
          '" name="' +
          text +
  ...
  $(document).on(
    "submit",
    "form[name=check-in], form[name=check-out]",
    function (evt) {
      evt.preventDefault();
  ...
  $(document).on(
    "click",
    "input[type=submit].save.check-in, input[type=submit].save.check-out",
    function (evt) {
      evt.preventDefault();
  ...
  function submit_check(from, t) {
    ...
    if (val == "0" || val == "") {
      alert(
        "Por favor, inserte un número de elementos para comprobar " + actpage
      );
      return false;
    }

    if (actpage == "in") var phppage = "check-in.php";
    else {
  ...
  // Manejador para la paginacion y mostrar por pagina
  function go(act, val) {
    ...
    if (actpage == "in") location.href = "check-in.php?" + url;
    else location.href = "check-out.php?" + url;
  }
