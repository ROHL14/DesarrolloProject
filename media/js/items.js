$("document").ready(function () {
  ...
  $(
    "tr[data-type=element] td[data-type=id], tr[data-type=element] td[data-type=name], tr[data-type=element] td[data-type=cat]"
  ).click(function () {
    var item = $(this).parent().data("id");
    location.href = "item.php?id=" + item;
  });

  // Checkbox de cualquier producto
  ...
  function check_items(act, t) {
    ...
    for (var i = 0; i < selected.length; i++) {
      ...
      if (
        $('input[type=checkbox][name=chbox][value="' + id + '"]')
          .parent()
          .parent()
          .next()
          .data("id") != "y"
        ...
        if (getViewportWidth() > 420)
          var cont = $(
            '<tr style="display:none" data-id="y"><td></td><td></td><td></td><td>' +
              text +
              '</td><td><form method="post" data-saveid="' +
              id +
              '" name="' +
              act +
              '"><input type="text" class="num" placeholder="Cant." /></form></td><td><input type="submit" value="Save" class="btn blue save ' +
              act +
              '" /></td><td></td></tr>'
          );
        else if (getViewportWidth() <= 420 && getViewportWidth() > 370)
          var cont = $(
            '<tr style="display:none" data-id="y"><td></td><td></td><td>' +
              text +
              '</td><td></td><td><form method="post" data-saveid="' +
              id +
              '" name="' +
              act +
              '"><input type="text" class="num" placeholder="Cant." /></form></td><td><input type="submit" value="Save" class="btn blue save ' +
          ...
          var cont = $(
            '<tr style="display:none" data-id="y"><td></td><td></td><td>' +
              ...
              '" name="' +
              act +
              '"><input type="text" class="num" placeholder="Cant." /></form></td><td></td><td><input type="submit" value="Guardar" class="btn blue save ' +
              act +
              '" /></td><td></td></tr>'
          );
  ...
  $("a[name=delete-all]").click(function (evt) {
    ...
    if (
      confirm(
        "Estas seguro que quieres eliminar " + selected.length + " productos?"
      )
    ) {
          ...
          for (var a = 0; a < decoded.length; a++) {
            ...
            $("input[name=chbox][value=" + item + "]")
              ...
              .fadeOut(700, function () {
                ...
                $(
                  "a[name=delete-all], a[name=check-out-all], a[name=check-in-all]"
                ).addClass("disabled");
              });
          }
        }
  ...
  // Check In/Out Manejador de un producto
      ...
      if (getViewportWidth() > 420)
        var cont = $(
          '<tr style="display:none" data-id="y"><td></td><td></td><td></td><td>' +
            ...
            '" name="' +
            act +
            '"><input type="text" class="num" placeholder="Cant" /></form></td><td><input type="submit" value="Guardar" class="btn blue save ' +
            act +
            '" /></td><td></td></tr>'
        );
      else if (getViewportWidth() <= 420 && getViewportWidth() > 370)
  ...
  // Valida solo valores numericos
  $(document).on(
    "keyup",
    "form[name=check-in] input.num, form[name=check-out] input.num",
    function (evt) {
      val = $(this).val();
      var t = $(this);
      var re = /^-{0,1}\d+$/;
      ...
      if (re.test(val) == false) t.val(val.substr(0, val.length - 1));
      return;
    }
  );
            ...
            .fadeOut(200, function () {
              if (act == 3)
                t.parent()
                  ...
                  .prev()
                  .children("td:nth-child(5)")
                  .html(parseInt(prev_value) + parseInt(val))
                  .fadeIn(400);
              else if (act == 4)
                t.parent()
                  .parent()
  ...
  $("a[name=c5]").click(function (evt) {
    ...
    if (confirm("¿Está seguro de que desea eliminar este elemento?") == true) {
      ...
      $.post(
        ...
        function (data) {
          if (data == "1") element.fadeOut(700);
          else alert("Algo salió mal. Por favor, inténtelo de nuevo");
        }
      );
    }
  });
  ...
  // Controlador de la paginacion y mostrar por pagina
  function go(act, val) {
  ...
  // Obtiene el ancho sin scrollbar
